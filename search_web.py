...
def configure_llm():
    ...
    llama_model = ChatOllama(
        model="llama3.2:1b", base_url="https://llm.bicbioeng.org", temperature=temperature)
    llama_model_json = ChatOllama(
        model="llama3.2:1b", format='json', base_url="https://llm.bicbioeng.org", temperature=temperature)
...
placeholder = st.empty()  # Create placeholder for streaming updates
...
def web_search(state):
    ...
    search_result = web_search_tool.invoke(
        f'{search_query} site:materialsproject.org')
